...
Status GuessP(double &PM,double DL,double DR,double UL,double UR,double PL,double PR,double CL,double CR,double GammaL, double GammaR)
//provide a guess value for pressure PM in the star region.AIRS approximate Riemann solvers.
{
	...
	const double QUSER=2.0, eps = 1e-10;
...
Status StarPU(double &P,double &U,double DL,double DR,double UL,double UR,double PL,double PR,double CL,double CR,double GammaL, double GammaR)
...
Status Sample(double &D,double &U,double &P, double &Gamma, double PM,double UM,double S,double DL,double DR,double UL,double UR,double PL,double PR,double CL,double CR,double GammaL, double GammaR)
//Sampling is performed in terms of the 'speed' S=X/T. Sampled values are D,U,P. PM and UM in the star region are known.
{
	...
	if(S<=UM)//(4)
			...
			if(PM<=PL)//(3)
				{// Left rarefaction
					...
					if(S<=SHL)//(2)
						...
						{
							CML=CL*pow(PM/PL,(GammaL-1.)/(2.*GammaL));
							STL=UM-CML;
							if(S>STL)//(1)
								...
								{//Sampled point is inside left fan
									U=2./(GammaL+1.)*(CL+(GammaL-1.)/2.*UL+S);
									C=2./(GammaL+1.)*(CL+0.5*(GammaL-1.)*(UL-S));
									D=DL*pow(C/CL,2./(GammaL-1.));
									P=PL*pow(C/CL,2.*GammaL/(GammaL-1.));
								}
						}
				}
			else//(3)
				{//Left shock
					PML=PM/PL;
					SL=UL-CL*sqrt((GammaL+1.)/(2.*GammaL)*PML+(GammaL-1.)/(2.*GammaL));
					//	printf("SL=%lf\n",SL);
					if(S<=SL)//(3)1
						...
						{//Sampled point is star left state
							D=DL*(PML+(GammaL-1.)/(GammaL+1.))/(PML*(GammaL-1.)/(GammaL+1.)+1.);
							U=UM;P=PM;
						}
				}
		}
		...
		{//Sampling point lies to the right os the contact discontinuity
			...
			if(PM>PR)//i
				{//Right Shock
					PMR=PM/PR;SR=UR+CR*sqrt((GammaR+1.)/(2.*GammaR)*PMR+(GammaR-1.)/(2.*GammaR));
					if(S>=SR)//ii
						...
						{//Sampled point is star right state
							D=DR*(PMR+(GammaR-1.)/(GammaR+1.))/((GammaR-1.)/(GammaR+1.)*PMR+1.);
							U=UM;P=PM;
						}
				}
			else//i
				{//Right rarefaction
					...
					if(S>=SHR)//iii
						...
						{
							CMR=CR*pow(PM/PR,(GammaR-1.)/(2.*GammaR));STR=UM+CMR;
							if(S<=STR)//iv
								{//Sampled point is star right state
								...
								{//sampled point is inside left fan
									U=2./(GammaR+1.)*(-CR+0.5*(GammaR-1.)*UR+S);
									C=2./(GammaR+1.)*(CR-0.5*(GammaR-1.)*(UR-S));
									D=DR*pow(C/CR,2./(GammaR-1.));
									P=PR*pow(C/CR,(2.*GammaR)/(GammaR-1.));
								}
						}
				}
...
Status GRPsolverES(double &DtD,double &DtU,double &DtP,double UM,double PM,double DL,double DR,double UL,double UR,double PL,
				   double PR,double DDL,double DDR,double DUL,double DUR,double DPL,double DPR,double TDSL,double TDSR,double DpsiL,double DphiR,double S,double r,double GammaL, double GammaR)
//GRP solver for non acoustic case.Euler version(moving mesh) spherical case
{
	double aL,bL,dL,aR,bR,dR,CL,CR,C_star,D,U,P,theta,SR,SL,phi1,phi2,phi3,sigmaL,sigmaR,musL=(GammaL-1.)/(GammaL+1.),musR=(GammaR-1.)/(GammaR+1.);
	CL=sqrt(GammaL*PL/DL);CR=sqrt(GammaR*PR/DR);
	double gD,gU,gP,H1,H2,H3,f,phic,hL,qL,kL,hR,qR,kR;
	if(PM<=PL)//Left rarefaction wave
		{
			// middle left state
			D=DL*pow(PM/PL,1./GammaL);U=UM;P=PM;
			C_star=sqrt(GammaL*P/D);theta=C_star/CL;
			aL=1.;bL=1./(D*C_star);
			if(GammaL==(5./3.))
				{
					phic=-2.*(3.*C_star*log(theta)-(-1.)*(UL+2.*CL/(GammaL-1.))*(1.-theta));
				}
			else
				{
					phic=(musL-1.)*C_star/(musL*(4.*musL-1.))*(1.-pow(theta,(1.-4.*musL)/(2.*musL)))+(UL+2.*CL/(GammaL-1.))/(2.*musL-1.)*(1.-pow(theta,(1.-2*musL)/(2.*musL)));
				}
			dL=((1.+musL)/(1.+2.*musL)*pow(theta,0.5/musL)+musL/(1.+2.*musL)*pow(theta,(1.+musL)/musL))*TDSL
				-pow(theta,0.5/musL)*CL*(DpsiL+(m-1.)/(2.*r)*(UL-S))+(m-1.)/(2.*r)*C_star*(phic-(U-S));
			hL=aL-D*(U-S)*bL;qL=bL-(U-S)/(D*C_star*C_star)*aL;
			kL=(m-1.)/r*(U-S)*(U-S)*hL+(C_star*C_star-(U-S)*(U-S))/(C_star*C_star)*dL;
			if(PM<=PR)//Right rarefaction wave
				{
					//middle right state
					D=DR*pow(PM/PR,1./GammaR);U=UM;P=PM;
					C_star=sqrt(GammaR*P/D);
					aR=1.;bR=-1./(D*C_star);theta=C_star/CR;
					if(GammaR==(5./3.))
						{
							phic=-2.*(3.*C_star*log(theta)-(1.)*(UR-2.*CR/(GammaR-1.))*(1.-theta));
						}
					else
						{
							//phic=(musR-1.)*C_star/(musR*(4.*musR-1.))*(1.-pow(theta,(1.-4.*musR)/(2.*musR)))+(UL+2.*CL/(Gamma-1.))/(2.*musR-1.)*(1.-pow(theta,(1.-2*musR)/(2.*musR)));
							phic=(musR-1.)*C_star/(musR*(4.*musR-1.))*(1.-pow(theta,(1.-4.*musR)/(2.*musR)))-(UR-2.*CR/(GammaR-1.))/(2.*musR-1.)*(1.-pow(theta,(1.-2*musR)/(2.*musR)));
						}
					//phic=(musR-1.)*C_star/(musR*(4.*musR-1.))*(1.-pow(theta,(1.-4.*musR)/(2.*musR)))-(UR-2.*CR/(Gamma-1.))/(2.*musR-1.)*(1.-pow(theta,(1.-2*musR)/(2.*musR)));
					dR=((1.+musR)/(1.+2.*musR)*pow(theta,0.5/musR)+musR/(1.+2.*musR)*pow(theta,(1.+musR)/musR))*TDSR
						+pow(theta,0.5/musR)*CR*(DphiR+(m-1.)/(2.*r)*(UR-S))+(m-1.)/(2.*r)*C_star*(phic+(U-S));
					...
					if(UM>=0.)
						{
							theta=C_star/CL;
							DtD=1./(C_star*C_star)*((kL*hR-kR*hL)/(qL*hR-qR*hL)+(GammaL-1)*D*(UM-S)*pow(theta,(1.+musL)/musL)*TDSL);
						}
					else
						{
							H1=DR*PR*(1-musR*musR)/((PR+musR*PM)*(PR+musR*PM));
							H2=DR*PM*(-1+musR*musR)/((PR+musR*PM)*(PR+musR*PM));
							H3=(PM+musR*PR)/(PR+musR*PM);
							sigmaR=(D*(U-S)-DR*(UR-S))/(D-DR);
				...
				{
					//middle Right state (behind shock)
					SR=DR*CR*sqrt(PM/PR*(GammaR+1.)/(2.*GammaR)+(GammaR-1.)/(2.*GammaR));
					D=DR*(PM/PR+(GammaR-1.)/(GammaR+1.))/(PM/PR*(GammaR-1.)/(GammaR+1.)+1.);
					U=UM;P=PM;C_star=sqrt(GammaR*P/D);
		...
		{
			//middle Left state(behind shock)
			SL=-CL*DL*(PM/PL*(GammaL+1.)/(2.*GammaL)+(GammaL-1.)/(2.*GammaL));
			D=DL*(PM/PL+(GammaL-1.)/(GammaL+1.))/(PM/PL*(GammaL-1.)/(GammaL+1.)+1.);
			U=UM;P=PM;C_star=sqrt(GammaL*P/D);
			...
			if(PM<=PR)//Right rarefaction wave
				{
					...
					//	phic=(musR-1.)*C_star/(musR*(4.*musR-1.))*(1.-pow(theta,(1.-4.*musR)/(2.*musR)))-(UR-2.*CR/(Gamma-1.))/(2.*musR-1.)*(1.-pow(theta,(1.-2*musR)/(2.*musR)));
					dR=((1.+musR)/(1.+2.*musR)*pow(theta,0.5/musR)+musR/(1.+2.*musR)*pow(theta,(1.+musR)/musR))*TDSL
						+pow(theta,0.5/musR)*CR*(DphiR+(m-1.)/(2.*r)*(UR-S))+(m-1.)/(2.*r)*C_star*(phic+(U-S));
					hR=aR-D*(U-S)*bR;qR=bR-(U-S)/(D*C_star*C_star)*aR;
					kR=(m-1.)/r*(U-S)*(U-S)*hR+(C_star*C_star-(U-S)*(U-S))/(C_star*C_star)*dR;
					DtU=(kL*qR-kR*qL)/(hL*qR-hR*qL);DtP=(kL*hR-kR*hL)/(qL*hR-qR*hL);
...
Status GRPsolverSS(double &DtD,double &DtU,double &DtP,double UM,double PM,double DL,double DR,double UL,double UR,double PL,
				   double PR,double DDR,double DUR,double DPR,double TDSL,double TDSR,double DpsiL,double DphiR,double S,double r,double GammaL,double GammaR)
//sonic case spherical case
{
	double dL,dR,C,CL,CR,C_star,C_starL,C_starR,D,U,P,theta,sigmaR,mus,Gamma;
	CL=sqrt(GammaL*PL/DL);CR=sqrt(GammaR*PR/DR);C_starL=CL*pow(PM/PL,(GammaL-1.)/(2.*GammaL));C_starR=CR*pow(PM/PR,(GammaR-1.)/(2.*GammaR));
...
Status GRPsolverSS1(double &DtD,double &DtU,double &DtP,double DL,double DR,double UL,double UR,double PL,
					double PR,double DDL,double DDR,double DUL,double DUR,double DPL,double DPR,double S,double r,double GammaL, double GammaR)//smooth
{
	double SHL,SHR,CL,CR;CL=sqrt(GammaL*PL/DL);CR=sqrt(GammaR*PR/DR);
	SHL=UL-CL;SHR=UR+CR;
	if(S<=SHL)//Left region
		{
			DtD=-(DL*DUL+(UL-S)*DDL+(UL-S)*DL*(m-1.)/r);
			DtU=-(DPL/DL+(UL-S)*DUL);
			DtP=-((UL-S)*DPL+GammaL*PL*DUL+GammaL*PL*(UL-S)*(m-1.)/r);
		}
	if(S>=SHR)//Right region
		{
			DtD=-(DR*DUR+(UR-S)*DDR+(UR-S)*DR*(m-1.)/r);
			DtU=-(DPR/DR+(UR-S)*DUR);
			DtP=-((UR-S)*DPR+GammaR*PR*DUR+GammaR*PR*(UR-S)*(m-1.)/r);
		}
	return OK;
}
#endif
